...
module.exports = {
  ...
  wordRegex: /[\\\!][a-zA-Z]+$/, // all prefixes start with a single `\` or `!` (!TEX) followed by letters ($ marks cursor position)
  ...
  getSuggestions({editor, bufferPosition, scopeDescriptor}) { // autocalled by `autocomplete-plus`
    ...
    if (this.prefix.length < 2) { return; }
  ...
  getPrefix(editor, bufferPosition) {
    ...
    let match = line.match(this.wordRegex); // returns array with the entire match as first element
    if (match) {
      return match[0];
    } else {
      return "";
