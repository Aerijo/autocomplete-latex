...
module.exports = { // deliberately chosen `undefined` for config properties problems to fail fast
  ...
  citationCompletions(prefix, editor) {
    let userBibFilePath = editor.getText().match(/% !T[eE]X bib =\s*(.*)/);
    if (!userBibFilePath) { return []; }
    let bibFilePath = path.resolve(editor.getDirectoryPath(), userBibFilePath[1]);
  ...
  parseBibFile(file, prefix) {
    ...
    for (let i = 0; i < entriesArray.length; i++) {
      ...
      suggestion.snippet = `\\\\autocite\\{${suggestion.displayText}\\}$1`;
  ...
  getPrefix(editor, bufferPosition) {
    ...
    if (match) {
      return match[0];
    } else if (match = line.match(this.citationPrefixRegex)) {
      return match[0];
    } else {
      return "";
    }
