...
module.exports = {
  ...
  enableDefaultCompletions: atom.config.get("autocomplete-latex.disableForScopeSelector"),
  ...
  loadProperties() {
    ...
    if (pathToUserCompletions) {
      USER_COMPLETIONS = require(path.resolve(pathToUserCompletions));
      if (!USER_COMPLETIONS) { return; }
    }

    if (enableDefaultCompletions) {
      COMPLETIONS = this.mergeObjects(DEFAULT_COMPLETIONS, USER_COMPLETIONS);
    } else {
          ...
          if (typeKey in first[scopeKey]) {
            for (var descriptorKey in second[scopeKey][typeKey]) {
              if (descriptorKey in first[scopeKey][typeKey]) {
                console.log(`${scopeKey}.${typeKey}.${descriptorKey} from user completions is interferring with the default completions. This user completion will be ignored.`);
                continue;
              } else {
