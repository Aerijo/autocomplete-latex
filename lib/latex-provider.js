...
module.exports = { // deliberately chosen `undefined` for config properties problems to fail fast if not set by loadProperties()
  ...
  loadProperties(serialized) {
    ...
    if (fs.existsSync(pathToUserCompletions)) { // if it can find the file
      try {
        fs.stat(pathToUserCompletions, (err, stats) => {
    ...
    if (serialized.cachedCompletions) {
      this.cache = serialized.cachedCompletions;
    } else {
      this.cache = new Map();
    }
  },
  ...
  getSuggestions({editor, bufferPosition, scopeDescriptor}) { // autocalled by `autocomplete-plus`
    ...
    if (this.cache.has(scopeDescriptor)) {
      completions = this.cache.get(scopeDescriptor);
    } else {
      ...
      this.cache.set(scopeDescriptor, completions);
    }
  ...
  parseBibFile(file, prefix) {
    ...
    let entriesArray = file.split(/\s*@\s*/); // Split by the @ sign into separate strings. Needs work, as contents could have @ in it
    let completions = [];
