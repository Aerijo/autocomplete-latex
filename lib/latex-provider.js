...
module.exports = {
  ...
  */
  ...
  loadProperties() {
    this.cache = new CacheManager(); // for general completions
    ...
    if (fs.existsSync(pathToUserCompletions)) { // if it can find the file
      try {
        if (path.extname(pathToUserCompletions) === ".cson") {
          userCompletions = CSON.readFileSync(pathToUserCompletions);
        } else {
          userCompletions = require(pathToUserCompletions);
        }
        ...
        */
        if (Object.getOwnPropertyNames(userCompletions)[0][0] === ".") {
          this.completionRegistry.addCompletion("userCompletions", userCompletions);
        } else {
          this.completionRegistry.addCompletionGroup(userCompletions);
        }
      }
      catch (err) {
    ...
    if (atom.config.get("autocomplete-latex.enableDefaultCompletions")) {
      defaultCompletions = require("./resources/completionsOrdered.json");
      this.completionRegistry.addCompletionGroup(defaultCompletions);
    }
  ...
  getSuggestions({editor, bufferPosition, scopeDescriptor}) { // autocalled by `autocomplete-plus`
    return new Promise((resolve) => { // reject is not supported
      ...
      // will be potentially deprecated if first-mate scope selectors added
  ...
  regenerateCompletions() {
    ...
    atom.notifications.addSuccess("You may need to reload Atom for changes to take effect");
  }
};
