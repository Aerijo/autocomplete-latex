...
module.exports = { // deliberately chosen `null` for config properties problems to fail fast if not set by loadProperties()
  ...
  getSuggestions({editor, bufferPosition, scopeDescriptor}) { // autocalled by `autocomplete-plus`
    ...
    if (this.packageCompletions && this.isUsePackage(line)) {
      ...
      return packageCompletions(this.packagePrefix, this.packageCache, this.emitter);
    }


    // generate prefix and return if not long enough
    this.prefix = this.getPrefix(line);
    let prefixFirstChar = this.prefix[0];

    if (this.prefix.length < this.minPrefixLength && prefixFirstChar !== "$") { return; }
    ...
    // Short list the remaining candidates
  ...
  regenerateCompletions() {
    ...
    this.completionRegistry.empty();
    this.completionRegistry.addCompletionGroup(require("./resources/completionsOrdered.json"));
    this.loadProperties();
    this.cache.empty();
